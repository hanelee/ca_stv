...
\begin{example}
    Consider the following 21 cast ballots, with voters' most preferred candidates at the top 
    down to their least preferred candidates at the bottom:
    ...
    The third round tabulation is as follows:
    \begin{itemize}
        \item A has five votes.
...
There are too many STV variations to cover in this report, 
but we refer readers to OpaVote for a more comprehensive overview.
\todo{Cite, https://opavote.com/methods/single-transferable-vote}
...
Other thresholds can be used, including some which updated dynamically
as the tabulation process progresses.
...
first-place vote count from the previous round of tabulation. \\ 
...
Whether or not STV is appropriate for a given locality is highly context-dependent.
Here we discusss the broad pros and cons of STV that apply to most implementations.
\subsection{Pros}

\paragraph{{\bf{Flexible magnitude:}}}
 STV can be used with or without districts. It is flexible enough to be used 
...
\paragraph{\bf{Proportional tendencies:}} 
STV in the multi-winner setting tends to produce more
proportional outcomes than plurality voting.\footnote{Though STV is not expressly proportional like, say, open list systems.} 
This is in part because STV dampens the 
...
STV also satisfies an axiom known as Proportionality for Solid Colations,
which says that if a group of voters is large enough to deserve $k$ seats and agrees on 
their $k$ preferred candidates, then they can expect to elect at least $k$ candidates.
However, voters rarely display such unanimity in their preferences in practice.\\

\paragraph{\bf{Non-partisan:}} 
 Unlike some list-based voting systems, STV does not require 
    candidates to run under a party label. This lowers the barrier to entry
    for candidates who may not enjoy the support of a political party, and makes it 
    suitable for local elections that are non-partisan.\\

\paragraph{\bf{Local alignment:}} STV is already used in six California cities to elect one or more winners, 
...
\subsection{Cons}
   ...
   \paragraph{\bf{Complex mechanism:}} STV is a relatively complex voting mechanism, and it can be difficult 
                    for voters to understand how their vote is converted into an outcome.  \\

    \paragraph{\bf{Voter burden:}} As a voting system, STV \emph{technically} requires that each voter provides 
    ...
    \paragraph{\bf{Vote exhaustion:}} If a voter does not complete their ballot, it is possible that 
                    through candidate elimination their vote will no longer be an active 
                    part of the tabulation process. \\

    \paragraph{\bf{Vote leakage:}} Political parties often express hesitation for STV,
    since votes that originally list a party member first can leak to other parties throughout 
...
\subsection{CA Single-subject rule}
California's ballot initiative process contains a single-subject rule that prohibits
a ballot initiative from ``embracing more than one subject." \todo{Cite, CA const art 2 sec 8d}
It would be relatively easy to implement STV through a ballot initiative without running
afoul of the single-subject rule.
The only concern is that it could be interpreted that the shift to multi-member districts 
is a distinct subject from the shift to STV.
But since STV as proposed requires muli-member districts, this seems unlikely to pose a problem.
...
using parameterized mathematical models. \todo{cite, mggg}
...
for voters: the impulsive voter, the deliberative voter, and the Cambridge voter.
...
The type of model---impulsive, deliberative, or Cambridge---determines the probability of 
the final ballot being generated.
The impulsive model simulates a voter who only considers one slate/candidate at a time.
The deliberative model simulates a voter who considers all the possible pairwise comparisons 
between the slates/candidates.
The Cambridge model uses actual historical data from Cambridge, MA to generate the ballot.

Consider the following example.
\begin{example}
Suppose we have divided the electorate into two blocs of voters, Hispanic (H) and non-Hispanic (NH).
Each bloc has a corresponding slate of three candidates.
The Hispanic slate prefers their own candidates 80\% of the time, while the non-Hispanic slate 
prefers their own candidates 70\% of the time.
Within each bloc and slate, the candidates are preferred equally.

Our models all begin by sampling an order of the slates. Consider a Hispanic voter. 
They might sample the slate order 
\[\begin{pmatrix}
    H\\
     H\\
     NH\\
     ...
     NH
\end{pmatrix}.\]
Then, within the Hispanic slate, they might sample the candidate order $H_1, H_3, H_2$, 
and within the non-Hispanic slate, they might sample the candidate order $NH_3, NH_2, NH_1$.
Thus the voter's ballot is 
\[B = \begin{pmatrix}
    H_1\\
    H_3\\
    NH_3\\
...
This ballot has different probabilities of being generated by the different models.
While the specific probabilities are not important, the impulsive model will sample this ballot
almost three times as often as the deliberative model.
